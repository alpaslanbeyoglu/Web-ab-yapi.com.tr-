...
import { BarChart3, AlertTriangle, ShieldCheck, Search, Calendar } from 'lucide-react';
...
export const Statistics: React.FC<StatisticsProps> = ({ stats, setActiveTab }) => {
  ...
  const translations = {
    tr: {
      ...
      desc: 'İstanbul genelinde yapı stoğu yaş dağılımı, risk haritası, kentsel yenileme hedefleri ve ilçe bazlı kentsel dönüşüm göstergeleri.',
      ...
      noteText: '1999 öncesi inşa edilen binaların yaklaşık %72\'sinde deniz kumu kullanımı ve yetersiz donatı detayları tespit edilmiştir. Bu binaların 6306 sayılı kanun kapsamında acilen yenilenmesi gerekmektedir.',
      tableTitle: 'İlçe Bazlı Kentsel Dönüşüm & Risk Tablosu',
      tableDesc: 'İstanbul ilçelerine göre tespit edilen riskli binalar ve ortalama bina yaşları',
      searchPlaceholder: 'İlçe Ara (Örn: Fatih)...',
      thDistrict: 'İlçe Adı',
      thRisky: 'Riskli Bina Sayısı',
      thRenewed: 'Yenilenen Bina',
      thAge: 'Ort. Bina Yaşı',
    ...
    en: {
      ...
      desc: 'Building stock age distribution, risk map, urban renewal targets, and district-based urban transformation indicators across Istanbul.',
      ...
      noteText: 'About 72% of buildings constructed before 1999 were found to have used sea sand and insufficient reinforcement details. These buildings must be renewed urgently under Law No. 6306.',
      tableTitle: 'District-Based Urban Transformation & Risk Table',
      tableDesc: 'Risky buildings detected according to Istanbul districts and average building ages',
      searchPlaceholder: 'Search District (e.g. Fatih)...',
      thDistrict: 'District Name',
      thRisky: 'Risky Buildings',
      thRenewed: 'Renewed Buildings',
    ...
    ar: {
      ...
      desc: 'توزيع أعمار المباني، خريطة المخاطر، أهداف التجديد الحضري، ومؤشرات التحول الحضري حسب الأحياء في إسطنبول.',
      ...
      noteText: 'تبين أن حوالي ٧٢٪ من المباني التي شيدت قبل عام ١٩٩٩ استخدمت رمال البحر وتفاصيل تسليح غير كافية. يجب تجديد هذه المباني بشكل عاجل بموجب القانون رقم 6306.',
      tableTitle: 'جدول التحول الحضري والمخاطر حسب المنطقة',
      tableDesc: 'المباني الخطرة المكتشفة حسب أحياء إسطنبول ومتوسط أعمار المباني',
      searchPlaceholder: 'البحث عن حي (مثال: الفاتح)...',
      thDistrict: 'اسم الحي',
      thRisky: 'عدد المباني الخطرة',
      thRenewed: 'المباني المجددة',
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 space-y-12">
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-md border border-slate-200 space-y-6">
        ...
        <div className="p-4 bg-amber-50 rounded-xl border border-amber-200 text-xs text-amber-900 space-y-1">
          <div className="font-bold">{activeTrans.noteTitle}</div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-xs font-medium text-slate-800 focus:outline-none focus:border-teal-600"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-100 text-slate-700 uppercase font-bold border-b border-slate-200">
              <tr>
                <th className="p-3.5">{activeTrans.thDistrict}</th>
                <th className="p-3.5 text-right">{activeTrans.thRisky}</th>
                <th className="p-3.5 text-right">{activeTrans.thRenewed}</th>
                <th className="p-3.5 text-center">{activeTrans.thAge}</th>
              ...
              {filteredDistricts.map((d, idx) => (
                <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                  <td className="p-3.5 font-bold text-slate-900 text-sm">
                    {language === 'ar' && d.district === 'Fatih' ? 'الفاتح' : language === 'en' && d.district === 'Fatih' ? 'Fatih' : d.district}
                  </td>
                  ...
                  <td className="p-3.5 text-center">
                    ...
                    >
                      {d.riskFactor === 'Yüksek' ? activeTrans.highRisk : activeTrans.medRisk}
                    </span>
                  </td>
                  <td className="p-3.5 text-right">
                    <button
                      onClick={() => setActiveTab('contact')}
                      className="bg-teal-50 hover:bg-teal-700 hover:text-white text-teal-800 font-bold px-3 py-1.5 rounded-lg transition-colors text-[11px] cursor-pointer"
                    >
